...
if __name__ == "__main__":
    ...
    print(train_graph_data.adj_matrix.shape)
    
    A = train_graph_data.adj_matrix.toarray()
    ...
    print(cnt)
    np.savetxt('cora_cnt.csv', cnt, fmt="%.4f", delimiter=',')
    ...
    while (True):
        if train_graph_data.is_epoch_end:
            ...
            if epochs % config.display == 0:
                ...
                norm_weights = None
                #by DM
                #check if adj_matrix is binary or not
                #print( train_graph_data.adj_matrix.toarray()[np.where(train_graph_data.adj_matrix.toarray() > 1)] )
