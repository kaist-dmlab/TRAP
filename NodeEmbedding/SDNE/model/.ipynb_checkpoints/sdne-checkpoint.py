...
class SDNE:
    def __init__(self, config):
        ...
        self.norm_weights = tf.Variable(tf.random_uniform([1, config.N]), name="norm_weights")
    ...
    def __make_compute_graph(self):
        def encoder(X):
            if self.config.sparse_reg == 0:
                print "sparse_reg : ", self.config.sparse_reg
                for i in range(self.layers - 1):
                    if i < (self.layers - 2):
                        name = "encoder" + str(i)
                ...
                for i in range(self.layers - 1):
                    if i < (self.layers - 2):
                        ...
                        name = "encoder" + str(i)
                        pre_mul = tf.transpose(tf.matmul(self.norm_weights, tf.transpose(self.input_OH)))  # (1, ?)
    ...
    def get_norm_weight(self, data, index):
        return self.sess.run(self.norm_weights, feed_dict=self.__get_feed_dict(data, index))  # feed 'index' together
