...
class Graph(object):
    def __init__(self, file_path, ng_sample_ratio):
        ...
        if suffix == "txt":
            ...
            #self.second_matrix = self.first2second(self.adj_matrix)
            #print(self.second_matrix[0])
        else:
            try:
    ...
    # by DM
    def first2second(self, adj_matrix):
        print "First2Second"
        F = adj_matrix.toarray()
        S = np.matmul(F,F)
        S = S+F
    ...
    def sample(self, batch_size, do_shuffle=True, with_label=False):
        ...
        #mini_batch.X = self.second_matrix[index]
        ...
        self.st = en
        # return mini_batch
        return mini_batch, index  # index by DM    
